...
export const DuneChart = ({ type, title, subtitle, executionID, queryID, state, index, isSQLGenerated }: Props & ChartProps) => {
  ...
  return <Box sx={{
    ...
    <Box sx={{ paddingTop: "16px", paddingLeft: "24px", paddingRight: "24px" }}>
      ...
      {isError && !isSQLGenerated && <Typography variant="subtitle1" sx={{ mt: 1 }}>Error: {error?.message}</Typography>}
    </Box>
    {isChartCounter &&
      <Typography
        variant="h2"
